...
feedbackForm.addEventListener("input", (0, _lodashThrottleDefault.default)(onInputForm, 500));
onReloadPage();
...
function onInputForm() {
    saveCurrentInput["email"] = inputEmail.value;
    saveCurrentInput["message"] = textareaMessage.value;
    localStorage.setItem(KEY_LOCAL, JSON.stringify(saveCurrentInput));
}
function onReloadPage() {
    const currentInput = JSON.parse(localStorage.getItem(KEY_LOCAL));
    if (currentInput) {
        inputEmail.value = currentInput.email ?? "";
        textareaMessage.value = currentInput.message ?? "";
    }
}
